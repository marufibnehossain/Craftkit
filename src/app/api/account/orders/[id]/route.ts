...
export async function GET(
  ...
  try {
    ...
    >("SELECT id, productId, name, priceCents, quantity, variationLabel FROM OrderItem WHERE orderId = ?", id);

    const productIds = [...new Set(items.map((i) => i.productId))];
    const products = productIds.length > 0
      ? await prisma.product.findMany({
          where: { id: { in: productIds } },
          select: { id: true, isDownloadable: true, downloadFiles: true },
        })
      : [];
    const productDownloadMap = new Map(products.map((p) => [p.id, p]));
    ...
    return NextResponse.json({
      ...
      items: items.map((i) => {
        const prod = productDownloadMap.get(i.productId);
        let downloadFiles: { name: string; url: string; size: number }[] | undefined;
        if (prod?.isDownloadable && prod.downloadFiles) {
          try { downloadFiles = JSON.parse(prod.downloadFiles); } catch {}
        }
        return {
          name: i.name,
          ...
          isDownloadable: prod?.isDownloadable ?? false,
          downloadFiles,
        };
      }),
    });
